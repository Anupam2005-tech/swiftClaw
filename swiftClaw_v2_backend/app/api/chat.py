...
async def stream_chat(
    ...
    # Map Firestore history to LangChain messages
    lc_history = []
    for msg in history:
        role = msg.get("role")
    ...
    # Determine max context tokens from capabilities registry
    max_context_tokens = 120000  # safe default
    pref_model = model_preferences.get(task_category)
    if pref_model and ":" in pref_model:
        prov, mod = pref_model.split(":", 1)
        caps = get_capabilities(prov, mod)
        if caps:
        ...
        try:
            ...
            while True:
                ...
                if chunk["type"] == "done":
                    ...
                    yield f"data: {json.dumps({'type': 'error', 'code': 'generation_error', 'message': chunk.get('message')})}\n\n"
                    break
