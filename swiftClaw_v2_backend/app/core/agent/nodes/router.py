...
from app.core.providers.factory import get_default_model
import structlog

logger = structlog.get_logger(__name__)


def get_model_for_provider(provider: str, prefs: dict, category: str) -> str:
    """Get the model for a provider from preferences or return default."""
...
async def router_node(state: AgentState) -> dict:
    ...
    model_gen = get_model_for_provider(provider_gen, prefs, category)
    ...
    if len(available) == 1:
        ...
        if other_providers:
            ...
            model_eval = get_model_for_provider(provider_eval, prefs, category)
        else:
            provider_eval = provider_gen
            model_eval = model_gen
